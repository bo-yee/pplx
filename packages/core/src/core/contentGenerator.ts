...
export type ContentGeneratorConfig = {
  ...
  // Web session token for cookie-based auth (WEB_SESSION auth type)
  sessionToken?: string;
};
...
export function createContentGeneratorConfig(
  ...
  if (authType === AuthType.WEB_SESSION) {
    return {
      ...
      // Pass session token through apiKey field for the WebContentGenerator
      apiKey: generationConfig?.sessionToken || generationConfig?.apiKey,
...
export async function createContentGenerator(
  ...
  // Web session token authentication (no API key needed)
  if (config.authType === AuthType.WEB_SESSION) {
    const { WebContentGenerator } = await import(
      './webContentGenerator/index.js'
    );
    return new WebContentGenerator(config, gcConfig);
  }

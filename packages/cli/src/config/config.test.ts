...
import * as ServerConfig from '@perplexity-cli/perplexity-cli-core';
...
vi.mock('@perplexity-cli/perplexity-cli-core', async () => {
  const actualServer = await vi.importActual<typeof ServerConfig>(
    '@perplexity-cli/perplexity-cli-core',
  );
  return {
    ...actualServer,
    IdeClient: {
...
describe('parseArguments', () => {
  ...
  it('should convert positional query argument to prompt by default', async () => {
    process.argv = ['node', 'script.js', 'Hi Perplexity'];
    const argv = await parseArguments({} as Settings);
    expect(argv.query).toBe('Hi Perplexity');
    expect(argv.prompt).toBe('Hi Perplexity');
    expect(argv.promptInteractive).toBeUndefined();
  });
  ...
  it('should map @path to prompt even when config flags are present', async () => {
    ...
    expect(argv.promptInteractive).toBeUndefined();
    expect(argv.model).toBe('perplexity-1.5-pro');
  });

  it('maps unquoted positional @path + arg to prompt (one-shot)', async () => {
    // Simulate: perplexity @path ./file.md
    process.argv = ['node', 'script.js', '@path', './file.md'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should handle multiple @path arguments in a single command (one-shot)', async () => {
    // Simulate: perplexity @path ./file1.md @path ./file2.md
    process.argv = [
      'node',
      'script.js',
      '@path',
  ...
  it('should handle mixed quoted and unquoted @path arguments (one-shot)', async () => {
    // Simulate: perplexity "@path ./file1.md" @path ./file2.md "additional text"
    process.argv = [
      'node',
      'script.js',
      '@path ./file1.md',
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/PERPLEXITY.md'],
      },
      {
        path: '/path/to/ext2',
        config: {
  ...
  // To re-enable tests for loadHierarchicalPerplexityMemory, ensure that:
  ...
  it.skip('should correctly use mocked homedir for global path', async () => {
    const MOCK_PERPLEXITY_DIR_LOCAL = path.join(
      '/mock/home/user',
      '.perplexity',
    );
    const MOCK_GLOBAL_PATH_LOCAL = path.join(
      MOCK_PERPLEXITY_DIR_LOCAL,
      'PERPLEXITY.md',
    );
    mockFs({
      [MOCK_GLOBAL_PATH_LOCAL]: { type: 'file', content: 'GlobalContentOnly' },
    });
    const memory = await loadHierarchicalPerplexityMemory(
      '/some/other/cwd',
      false,
    );
...
describe('loadCliConfig model selection', () => {
  ...
  it.skip('uses the default perplexity model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe(DEFAULT_PERPLEXITY_MODEL);
  });

  it('always prefers model from argvs', async () => {
    process.argv = ['node', 'script.js', '--model', 'sonar-pro'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {
        model: {
          name: 'perplexity3-coder-flash',
        },
      },
      [],
      new ExtensionEnablementManager(
...
describe('loadCliConfig with includeDirectories', () => {
  ...
  it('should use --project as the target directory and resolve relative include directories from it', async () => {
    const launchCwd = process.cwd();
    const projectDir = path.resolve(path.sep, 'home', 'user', 'project');
    process.argv = [
      'node',
      'script.js',
      '--project',
    ...
    expect(resolveProjectDirectory(argv, launchCwd)).toBe(projectDir);
    expect(config.getTargetDir()).toBe(projectDir);
    expect(config.getWorkingDir()).toBe(projectDir);
    expect(config.getWorkspaceContext().getDirectories()).toEqual(
      expect.arrayContaining([projectDir, path.join(projectDir, 'cli/path2')]),
    );
  });
});
...
describe('loadCliConfig interactive', () => {
  ...
  it('should be interactive if no positional prompt words are provided with flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'perplexity-1.5-pro'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {},
      [],
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectPerplexityIgnore',
      getter: (c) => c.getFileFilteringRespectPerplexityIgnore(),
      value: true,
    },
    {
      property: 'respectPerplexityIgnore',
...
describe('Telemetry configuration via environment variables', () => {
  it('should prioritize PERPLEXITY_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = { telemetry: { enabled: false } };
    const config = await loadCliConfig(
  ...
  it('should prioritize PERPLEXITY_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should throw when PERPLEXITY_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should prioritize PERPLEXITY_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv(
      'PERPLEXITY_TELEMETRY_OTLP_ENDPOINT',
      'http://perplexity.env.com',
    );
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
    ...
    expect(config.getTelemetryOtlpEndpoint()).toBe('http://perplexity.env.com');
  });

  it('should prioritize PERPLEXITY_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should prioritize PERPLEXITY_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = { telemetry: { logPrompts: true } };
    const config = await loadCliConfig(
  ...
  it('should prioritize PERPLEXITY_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_OUTFILE', '/perplexity/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
    ...
    expect(config.getTelemetryOutfile()).toBe('/perplexity/env/telemetry.log');
  });

  it('should prioritize PERPLEXITY_TELEMETRY_USE_COLLECTOR over settings', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_USE_COLLECTOR', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should use settings value when PERPLEXITY_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = { telemetry: { enabled: true } };
    const config = await loadCliConfig(
  ...
  it('should use settings value when PERPLEXITY_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = { telemetry: { target: 'local' } };
    const config = await loadCliConfig(
  ...
  it("should treat PERPLEXITY_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {},
  ...
  it("should treat PERPLEXITY_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
  ...
  it("should treat PERPLEXITY_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {},
  ...
  it("should treat PERPLEXITY_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('PERPLEXITY_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(

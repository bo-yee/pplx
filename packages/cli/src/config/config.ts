...
import {
  ...
  getCurrentPerplexityMdFilename,
  loadServerHierarchicalMemory,
  setPerplexityMdFilename as setServerPerplexityMdFilename,
  ShellTool,
  WriteFileTool,
...
export async function parseArguments(settings: Settings): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    .locale('en')
    .scriptName('perplexity')
    .usage(
      'Usage: perplexity [options] [command]\n\nPerplexity Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description:
        'Proxy for Perplexity Code, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command(
      ...
      (yargsInstance: Argv) =>
        yargsInstance
          ...
          .option('project', {
            alias: ['project-dir', 'cwd'],
            type: 'string',
            description:
              'Project directory to use as the workspace root. Relative paths are resolved from the launch directory.',
          })
          ...
          .option('web-search-default', {
            type: 'string',
            description:
              'Default web search provider (perplexity, tavily, google)',
          })
          ...
          .option('auth-type', {
            type: 'string',
            choices: [AuthType.USE_OPENAI, AuthType.PERPLEXITY_API_KEY],
...
export function resolveProjectDirectory(
  argv: Pick<CliArgs, 'project'>,
  launchCwd: string = process.cwd(),
): string {
  const requestedProject = argv.project?.trim();
  if (!requestedProject) {
    return fs.realpathSync(path.resolve(launchCwd));
  }

  const expandedProject = resolvePath(requestedProject);
  const resolvedProject = path.isAbsolute(expandedProject)
    ? expandedProject
    : path.resolve(launchCwd, expandedProject);

  if (!fs.existsSync(resolvedProject)) {
    throw new FatalConfigError(
      `Project directory does not exist: ${resolvedProject}`,
    );
  }

  if (!fs.statSync(resolvedProject).isDirectory()) {
    throw new FatalConfigError(
      `Project path is not a directory: ${resolvedProject}`,
    );
  }

  return fs.realpathSync(resolvedProject);
}
...
function resolveWorkspaceDirectory(
  ...
  const expanded = resolvePath(directory.trim());
  if (!expanded) {
    return '';
  }
...
export async function loadHierarchicalPerplexityMemory(
  currentWorkingDirectory: string,
  includeDirectoriesToReadPerplexity: readonly string[] = [],
  debugMode: boolean,
  ...
  return loadServerHierarchicalMemory(
    effectiveCwd,
    includeDirectoriesToReadPerplexity,
    debugMode,
    fileService,
...
export async function loadCliConfig(
  ...
  const projectDir = resolveProjectDirectory(argv, cwd);
  ...
  // directly to the Config constructor in core, and have core handle setPerplexityMdFilename.
  // However, loadHierarchicalPerplexityMemory is called *before* createServerConfig.
  if (settings.context?.fileName) {
    setServerPerplexityMdFilename(settings.context.fileName);
  } else {
    // Reset to default if not provided in settings.
    setServerPerplexityMdFilename(getCurrentPerplexityMdFilename());
  }
  ...
  const fileService = new FileDiscoveryService(projectDir);
  ...
  const includeDirectories = (settings.context?.includeDirectories || [])
    .map((dir) => resolveWorkspaceDirectory(dir, projectDir))
    .concat(
      (argv.includeDirectories || []).map((dir) =>
        resolveWorkspaceDirectory(dir, projectDir),
      ),
    )
    .filter(Boolean);

  // Call the (now wrapper) loadHierarchicalPerplexityMemory which calls the server's version
  const { memoryContent, fileCount } = await loadHierarchicalPerplexityMemory(
    projectDir,
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`perplexity --resume\` without an ID to choose from existing sessions.`;
        console.log(message);
        process.exit(1);
      }
    }
  ...
  return new Config({
    sessionId,
    sessionData,
    embeddingModel: DEFAULT_PERPLEXITY_EMBEDDING_MODEL,
    ...
    perplexityMdFileCount: fileCount,
    approvalMode,
    showMemoryUsage:
    ...
    authType:
      ...
      // Auto-detect: if session token is set and no API key, use web session auth
      (resolveSessionToken(settings) && !resolveApiKey(argv, settings)
    ...
    generationConfig: {
      ...
      sessionToken: resolveSessionToken(settings),
      baseUrl:
        argv.openaiBaseUrl ||
...
/**
 * Resolve session token from env or settings.
 */
function resolveSessionToken(settings: Settings): string | undefined {
  return (
    process.env['PERPLEXITY_SESSION_TOKEN'] ||
    settings.security?.auth?.sessionToken ||
    undefined
  );
}

/**
 * Resolve API key from argv, env, or settings.
 */
function resolveApiKey(argv: CliArgs, settings: Settings): string | undefined {
  return (
    argv.openaiApiKey ||
    process.env['PERPLEXITY_API_KEY'] ||
    settings.security?.auth?.apiKey ||

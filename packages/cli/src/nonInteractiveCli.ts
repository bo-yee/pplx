...
import { isAtCommand, isSlashCommand } from './ui/utils/commandUtils.js';
...
import { handleLocalPathMentions } from './ui/hooks/localPathProcessor.js';
import {
  handleError,
  handleToolError,
...
export async function runNonInteractive(
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    const perplexityClient = config.getPerplexityClient();
    ...
    try {
      ...
      if (!initialPartList) {
        ...
        if (!slashHandled) {
          const { processedQuery, shouldProceed } = isAtCommand(input)
      ...
      while (true) {
        ...
        const responseStream = perplexityClient.sendMessageStream(
        ...
        for await (const event of responseStream) {
          ...
          if (adapter) {
            ...
            if (event.type === PerplexityEventType.ToolCallRequest) {
              toolCallRequests.push(event.value);
            }
          } else {
            // Text output mode - direct stdout
            if (event.type === PerplexityEventType.Thought) {
              process.stdout.write(event.value.description);
            } else if (event.type === PerplexityEventType.Content) {
              process.stdout.write(event.value);
            } else if (event.type === PerplexityEventType.ToolCallRequest) {
              toolCallRequests.push(event.value);
            } else if (event.type === PerplexityEventType.Error) {

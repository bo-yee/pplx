...
export async function runAcpAgent(
  ...
  new acp.AgentSideConnection(
    (client: acp.Client) =>
      new PerplexityAgent(config, settings, extensions, argv, client),
    stdout,
    stdin,
  );
}
...
class PerplexityAgent {
  ...
  async initialize(
    ...
    const authMethods = [
      ...
      },
      {
        ...
        name: 'Perplexity OAuth',
        description:
          'OAuth authentication for Perplexity models with 2000 daily requests',
      },
      {
        id: AuthType.WEB_SESSION,
        name: 'Web Session Token',
        description:
          'Cookie-based authentication using browser session token',
      },
    ];
  ...
  async authenticate({ methodId }: acp.AuthenticateRequest): Promise<void> {
    ...
    if (method !== AuthType.WEB_SESSION) {
  ...
  private async ensureAuthenticated(config: Config): Promise<void> {
    ...
    // Auto-detect web session token from environment
    if (!selectedType && process.env['PERPLEXITY_SESSION_TOKEN']) {
      await config.refreshAuth(AuthType.WEB_SESSION);
      return;
    }
  ...
  private async createAndStoreSession(
    ...
    const history = conversation
      ...
      : undefined;
    const chat = history
      ? await perplexityClient.startChat(history)
      : await perplexityClient.startChat();

...
import type { Config, AuthType } from '@perplexity-cli/perplexity-cli-core';
import {
  InputFormat,
  logUserPrompt,
...
function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  if (process.env['PERPLEXITY_CLI_NO_RELAUNCH']) {
    return [];
  }
...
export async function main() {
  ...
  let settings = loadSettings(launchCwd);
  migrateDeprecatedSettings(settings);
  await cleanupCheckpoints();

  let argv = await parseArguments(settings.merged);
  let workspaceRoot = cliConfig.resolveProjectDirectory(argv, launchCwd);
  if (workspaceRoot !== launchCwd) {
    process.chdir(workspaceRoot);
    settings = loadSettings(workspaceRoot);
    migrateDeprecatedSettings(settings);
    argv = await parseArguments(settings.merged);
    workspaceRoot = cliConfig.resolveProjectDirectory(argv, launchCwd);
  }
  ...
  // to run Perplexity CLI. It is now safe to perform expensive initialization that
  // may have side effects.
  {
    ...
    if (!input) {
      console.error(
        `No input provided via stdin. Input can be provided by piping data into perplexity or using the --prompt option.`,
      );
      process.exit(1);
    }

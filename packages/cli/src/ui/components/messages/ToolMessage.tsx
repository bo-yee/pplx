...
import { PerplexityRespondingSpinner } from '../PerplexityRespondingSpinner.js';
...
export const ToolMessage: React.FC<ToolMessageProps> = ({
  ...
  // Add logging for resultDisplay
  React.useEffect(() => {
    if (config?.getDebugMode()) {
      const displayType = Array.isArray(resultDisplay)
        ? 'AnsiOutput'
        : typeof resultDisplay;
      const displayLength = Array.isArray(resultDisplay)
        ? resultDisplay.length
        : (resultDisplay as string)?.length;
      console.log(
        `[ToolMessage] '${name}' resultDisplay changed. Type: ${displayType}, Length: ${displayLength}, Status: ${status}`,
      );
      // Optionally log a snippet of the content if it's a string
      if (typeof resultDisplay === 'string') {
        console.log(
          `[ToolMessage] Snippet: ${resultDisplay.substring(0, 100)}...`,
        );
      }
    }
  }, [resultDisplay, name, status, config]);

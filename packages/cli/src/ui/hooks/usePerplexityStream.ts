...
import { handleLocalPathMentions } from './localPathProcessor.js';
  ...
  const prepareQueryForPerplexity = useCallback(
    async (
      ...
      if (typeof query === 'string') {
        ...
        // Handle @-commands and explicit local path mentions before sending to Perplexity.
        if (isAtCommand(trimmedQuery)) {
          ...
          const localPathResult = await handleLocalPathMentions({
            query: trimmedQuery,
            config,
            addItem,
            onDebugMessage,
          ...
          if (!localPathResult.shouldProceed) {
            return { queryToSend: null, shouldProceed: false };
          }
          localQueryToSendToPerplexity = localPathResult.processedQuery;
        }
      } else {
